    ...
    ADIO_Offset offset, ADIO_Status *status, int *error_code)
{
    ...
    if (fd->hints->cb_read != ADIOI_HINT_DISABLE) {
         ...
         */
        if ((get_oceanfsmpio_read_aggmethod() == OCEANFS_1) || (get_oceanfsmpio_read_aggmethod() == OCEANFS_2)) {
    ...
    ADIO_Offset offset, ADIO_Status *status, int *error_code)
{
    ...
    ad_oceanfs_timing_report(fd, OCEANFSMPIO_CIO_R_STRIDED_COLL, t);
    return;
}
