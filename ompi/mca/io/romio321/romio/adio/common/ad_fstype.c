...
# define OCEANFS_DPC_MAGIC 0xFFEA36969
# define OCEANFS_DTFS_MAGIC 0xFFEA36968
#endif
...
# ifdef ROMIO_OCEANFS
    if (((fsbuf.f_type == OCEANFS_DPC_MAGIC)||(fsbuf.f_type == OCEANFS_DTFS_MAGIC))&&(get_nasmpio_oceanfs()==1)) {
        *fstype = ADIO_OCEANFS;
        return;
    }
# endif
...
static void ADIO_FileSysType_prefix(const char *filename, int *fstype, int *error_code)
{
	...
	*fstype = ADIO_PVFS2;
	    ...
	    *fstype = ADIO_ZOIDFS;
		    ...
		    !strncmp(filename, "OCEANFS:", 8))&& (get_nasmpio_oceanfs()==1)) {
	    *fstype = ADIO_OCEANFS;
    } 
